...
const isTeacherRole = (role?: string) => (role || "").toLowerCase().includes("enseign");
...
function ChatWorkspace({
    ...
    const { contacts, loading: loadingContacts } = useChatContacts(currentUserId);
    ...
    } = useChat(currentUserId, contacts);
    ...
    const content = (
        <div className="space-y-4">
            <div className="flex items-center justify-between gap-3 flex-wrap">
                ...
                <Link
                    to={isTeacherRole(user.role) ? "/enseignant" : "/dashboard"}
                    className="px-3 py-2 rounded-xl border border-navy/15 text-sm font-semibold text-navy hover:bg-ice"
                >
                    Retour
                </Link>
            </div>
            <ChatWorkspace currentUserId={user.id} />
        </div>
    );

    if (isTeacherRole(user.role)) {
        return (
            <div className="min-h-screen bg-ice text-navy font-sans p-4 md:p-6">
                {content}
            </div>
        );
    }

...
const ParentList: React.FC<ParentListProps> = ({ refreshKey, onEdit, onView, searchTerm = "" }) => {
    const { parents, loading, error, refresh } = useParents(refreshKey);
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            setIsDeleting(true);
            await parentService.deleteParent(deleteConfirm.id);
    ...
    if (loading) {
        return (
            <div className="bg-white rounded-2xl p-6 shadow-card">
                <p className="text-slate text-sm">Chargement des parents...</p>
            </div>
        );
    }
    ...
    return (
        <div className="bg-white rounded-2xl p-6 shadow-card animate-fadeUp">
            <div className="flex items-start justify-between mb-5">
                <div>
                    <h3 className="font-display text-[16px] font-semibold text-navy">Liste des parents</h3>
                    <p className="text-slate text-[12px] mt-0.5">Suivi des comptes et enfants liés</p>
                </div>
                <span className="bg-teal/10 text-teal text-[11px] font-bold px-2.5 py-1 rounded-full">{filteredParents.length}</span>
            </div>
                ...
                <table className="w-full">
                    <thead>
                        <tr className="border-b-2 border-ice">
                            <th className="text-left text-[11px] font-semibold tracking-wider uppercase text-slate pb-3 pl-2">Parent</th>
                            <th className="text-left text-[11px] font-semibold tracking-wider uppercase text-slate pb-3 px-2">Enfants</th>
                    ...
                    <tbody>
                        {filteredParents.map((item, index) => {
                            ...
                            return (
                                <tr key={item.id} className={`hover:bg-ice transition-colors ${index !== filteredParents.length - 1 ? "border-b border-navy/5" : ""}`}>
                                    <td className="py-3 pl-2 pr-2">
                                        <div className="flex items-center gap-2.5">
                                            <div className="w-8 h-8 rounded-full flex items-center justify-center text-[11px] font-bold text-teal bg-teal/10 flex-shrink-0">{initials || "PR"}</div>
                                            <span className="text-[13px] font-medium text-navy">{item.prenom} {item.nom}</span>
                                        </div>
                                    </td>
                                    <td className="py-3 px-2 text-[13px] text-navy">{item.childIds?.length ?? 0}</td>
                        ...
                        {filteredParents.length === 0 && (
                            <tr>
                                <td colSpan={4} className="py-8 text-center text-[13px] text-slate">Aucun parent trouvé pour "{searchTerm}".</td>
                            </tr>
                        )}
                    </tbody>
                </table>
            ...
            {deleteConfirm && (
                <div className="fixed inset-0 bg-navy/60 flex items-center justify-center z-50 animate-fadeIn">
                    <div className="bg-white rounded-2xl p-6 max-w-md w-full mx-4 shadow-xl animate-slideUp">
                        ...
                        <p className="text-slate text-sm">Voulez-vous vraiment supprimer le parent <span className="font-semibold text-navy">{deleteConfirm.prenom} {deleteConfirm.nom}</span> ?</p>
